        ...
        <div class="w-full max-w-full px-3 mt-0  lg:flex-none">
            <div
                ...
                <div class="border-black/12.5 mb-0 rounded-t-2xl border-b-0 border-solid p-6 pt-4 pb-0">
                    <h6 class="capitalize ">Deadline Pengumpulan Berkas</h6>

                </div>
                <div class="flex-auto p-4">
                    <form method="POST" action="{{ route('admin.team.store',['id'=>$data->id]) }}">
                        @method('PUT')
                        @csrf
                        <div class="relative">
                            <input type="hidden" name="team_status" value="{{$data->name_team}}">
                            <label for="status" class="block mb-2 text-sm font-medium text-gray-900 ">Ubah Posisi Taeam {{$data->name_team}}</label>
                            <select id="status"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 mb-2"name="team_status">
